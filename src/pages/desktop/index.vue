  ...
  <div class="home">
    ...
    <div class="headers">
      <ul class="headers-left">
        ...
        <li class="iconfont" style="font-size: 14px" @click="showModal">
          壁纸
        </li>
        <li class="iconfont" style="font-size: 14px" @click="screen">大屏</li>
      ...
      <ul class="headers-right">
        <li class="iconfont" @click="personCenter">
          <span style="font-size: 14px"
            ><a-avatar
              ...
              :src="userinfo.user.avatar.url || avatar"
            />{{ userinfo.user.nickname }}</span
          >
        </li>
        <li class="iconfont">
          <a-popconfirm
        ...
        <li
          ...
          "
        >
          <a-popover trigger="click" v-model="visiblePopover">
            <div slot="title">
              <span>最新消息</span>
              <a @click="() => (visiblePopover = false)" style="float: right">关闭</a>
            </div>
            <a-badge :count="messages.total">
              <a><a-icon type="bell" /></a>
            </a-badge>
            <a-list
              item-layout="horizontal"
              :data-source="messages.items"
              :loading="loading"
              :size="'small'"
              slot="content"
            >
              ...
              >
                <a-button type="link" @click="onMessage"> 查看更多 </a-button>
              </div>
              <a-list-item slot="renderItem" slot-scope="item, index">
                <a-list-item-meta :key="index">
                  <span slot="title">
                    ...
                    <br />
                    <a @click="onMessageDetails(item)" style="color: #1890ff">
                      {{item.title | ellipsis}}
                    </a>
                  </span>
                  <div slot="description">
                    {{ item.details | ellipsis }}
                  </div>
                  <div slot="avatar">
                    <a-tag color="#ff0000" v-if="!item.isread"> 未读 </a-tag>
                    <a-tag color="#87d068" v-else> 已读 </a-tag>
                  </div>
                </a-list-item-meta>

                <a-tooltip
                  :title="`跳转到 -- > ${item.application.title}`"
                ...
                >
                  <a
                    style="color: #1890ff"
                    @click="openApplications(item.application)"
                    >{{ item.application.title }}</a
                  >
                </a-tooltip>
                <template v-else slot="actions"> N/A </template>
              </a-list-item>
            </a-list>
          </a-popover>
        </li>
    ...
    <div
      ...
      class="content-hone"
      :style="{ backgroundImage: lisr ? 'url(' + bgSrc + ')' : '' }"
      @click="weatherHide"
    >
      <ul class="app-list">
        <li
    ...
    <div class="desktop_topright" v-if="displays">
      <a-spin tip="Loading..." :spinning="spinning">
        ...
        <Schedule
          :events="events"
          :maximize="onScheduleMaximize"
          :hidden="() => (displays = false)"
          @onItemClick="onScheduleItemClick"
          @refresh="onScheduleRefresh"
        ></Schedule>
      </a-spin>
    </div>
    <!-- 桌面底部列表 -->
          ...
          <template v-for="item in bottoms">
            <li @dblclick="openApplication(item)" :key="item.id">
              ...
              <a href="javascript:void(0)"
                ><img :src="item.icon.url || iconUrl"
              /></a>
            </li>
          </template>
          <a-divider
            type="vertical"
            style="height: 45px; background-color: #bbbbbb; width: 2px"
            v-if="apps && apps.length && bottoms.length"
          />
          <a-popover
...
export default {
  ...
  methods: {
    ...
    screen() {
      const{ href } = this.$router.resolve({name:"screen"});
