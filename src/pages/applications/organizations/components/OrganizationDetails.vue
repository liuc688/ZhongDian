 ...
 -->
    ...
    >
      ...
      <span slot="policy" slot-scope="{ record }">
        <a-tag color="cyan" v-if="record.policy">
          {{ record.policy.title }}
        </a-tag>
        <div v-else></div>
      </span>
    ...
    >
      <a-alert
        message="密码长度为8-16位,必须以数字、英文、特殊字符至少两种的组合形式"
        type="warning"
        show-icon
      >
      </a-alert>
...
import md5 from "md5";
import { policy_tree } from "@/services/policy";
...
export default {
  ...
  methods: {
      ...
      organization_tree({ id: e.target.value }).then((res) => {
        ...
        if (e.target.checked) {
          for (let index = 0; index < result.length; index++) {
            if (this.organizationes.indexOf(result[index]) == -1) {
              this.organizationes.push(result[index]);
            }
          }
        } else {
          const datas_s = [];
          for (let index = 0; index < this.organizationes.length; index++) {
            if (result.indexOf(this.organizationes[index]) == -1) {
              datas_s.push(this.organizationes[index]);
            }
          }
          this.organizationes = datas_s;
        }
      });
    },
    gets(data) {
    ...
    init() {
      ...
      // 策略树
      if (this.params.id) {
        policy_tree({ organization: this.params.id })
          .then((response) => {
