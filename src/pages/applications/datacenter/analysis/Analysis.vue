    ...
    <a-spin tip="加载中..." :spinning = 'spinning'>
      <a-row style="margin-top: 0" :gutter="[24, 24]">
        <a-col :sm="24" :md="12" :xl="6">
          <chart-card title="总营业额 （万元）" :total="turnoverObj.yingyee>=0?turnoverObj.yingyee.toFixed(2).replace(this.reg,'$&,'):''">
          </chart-card>
        </a-col>
        <a-col :sm="24" :md="12" :xl="6">
          <chart-card  title="总纳税额 （万元）" :total="turnoverObj.nashui>=0?turnoverObj.nashui.toFixed(2).replace(this.reg,'$&,'):''">
          </chart-card>
        </a-col>
        <a-col :sm="24" :md="12" :xl="6">
    ...
    init(){
      ...
      .then(res => {
        ...
        this.TurnoverRanking = this.TurnoverRanking.map(item => {
          return {
            name: item.id.title,
            total:item.count>=0?(item.count.toFixed(2).replace(this.reg,'$&,')):"",
            id:item.id.id
          }
        })
      })
...
<style lang="less" scoped>
  ...
  #zhongdiancontainer{
    ...
    height: 450px;
    margin: 10px auto;
    box-shadow: #000000b4 0px 0px 8px;
    border-radius: 8px;
  }
</style>
<style>
