        ...
        <div v-if="visble_business" style="padding-top: 20px">
          <a-descriptions class="descriptions_es descriptions_s">
            ...
            <a-descriptions-item label="行业">
              <a-tag v-for="item in businessform.oneIndustry ?  businessform.oneIndustry.split(',') :[]" :key="item">{{
                item
              }}</a-tag>
            </a-descriptions-item>
          </a-descriptions>
        ...
        <div v-else style="margin-top: 20px">
          ...
          >
            <a-row>
              ...
              <a-col :span="12">
                <a-form-model-item class="fontsize_label">
                  <span slot="label" class="fontsize_label"><span style="color:red">*</span> 行业</span>
                  <a-tree-select
                    v-model="editoneIndustry"
                    multiple
                    allowClear
      ...
      <a-card class="business_info" :loading="loading">
        ...
        <div v-if="visble_honor">
          <div style="margin: 20px 5%">
            <div class="imgwalls" v-for="(itm, index) in qyzzry" :key="'qyzzry' + index">
...
export default {
  ...
  data() {
    return {
      ...
      lands: [],
      businessform: {
        // oneIndustry:""  // 行业字段对应的是一个空字符串，表示当修改 oneIndustry（行业字段）的时候，vue 可以监听的到，不然，我们要使用 this.$set 去添加元素
      },
      currentImg: "",
      preImages: [],
      previewShow: false,
  ...
  methods: {
    ...
    submitbusiness() {
      this.$refs.businessform.validate((valid) => {
        if (valid) {
          if(this.editoneIndustry.length){
            var datas = Object.assign(this.businessform, {
            oneIndustry: this.editoneIndustry.join(","),
            company: this.id,
            comeform: "PC端",
          });
    ...
    edit_business() {
      ...
      this.$nextTick(()=>{
        this.editoneIndustry = this.businessform.oneIndustry ? this.businessform.oneIndustry.split(',') : []
      })
    },
