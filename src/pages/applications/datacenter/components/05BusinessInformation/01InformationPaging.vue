    ...
    <div class="task_list">
      ...
      >
        ...
        <span slot="years" slot-scope="text, record">
          ...
          -->
          <a-tag :color='colorList[(record.release?record.release.split("-")[0]%12:"")]' style="font-weight:600">
            {{record.release?record.release.split("-")[0]:"无"}}
          </a-tag>
        </span>
        <!-- 月份字段的插槽(样式自定义) -->
        <span slot="release" slot-scope="text, record">
          ...
          <span v-else>
            <a-tag color="" style="font-weight:600">
              无
            </a-tag>
          </span>
        </span>
...
export default {
  ...
  methods: {
    ...
    toDetails(record) {
      // 只有当该条数据的 manage 字段不为 None 的时候，才能跳转页面
      if(record.manage!=="None") {
        this.$emit("event", {
          method: "onLink",
          params: {
            route: "managedetails",
      ...
      }else{  // 否则提示用户去创建 1 条经营信息
        this.$message.warn("请先为该企业创建 1 条经营信息")
      }
    },
    // 管委会页面（包括走访记录、点评等等...）
    ...
    disabledDate(current) {
      console.log('----',current)
      // return (new Date(current).getMonth() + 1 > new Date(moment().endOf("month")).getMonth());
      // 返回当前日期并且当前日期大于
